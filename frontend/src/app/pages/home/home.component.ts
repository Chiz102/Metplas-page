    ...
    <section class="section carousel-section">
      <div class="container">
        ...
        <div class="carousel-wrapper" (mouseenter)="onCarouselHover(true)" (mouseleave)="onCarouselHover(false)">
          <div class="carousel-container" [style.--carousel-items-visible]="carouselItemsVisible">
...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  carouselItemsVisible = 3; // Number of items to show at once
  ...
  get translatePercent(): number {
    // Calculate the translation percentage per step (100 / visible items)
    return 100 / this.carouselItemsVisible;
  }

  get carouselItemsWithClones() {
    // Original items + clone of first N items at the end for seamless loop
    return [...this.carouselItems, ...this.carouselItems.slice(0, this.carouselItemsVisible).map(item => ({ ...item, isClone: true }))] as CarouselItem[];
  }
  ...
  carouselNext() {
    ...
    // When index reaches the end of original items (clones start showing)
    if (this.carouselIndex === (this.carouselItems.length / this.carouselItemsVisible) +2) {
